...
#define NODE_ID   1        // compile-time role: 0 = master (board A), 1 = slave (board B)
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcanp)
{
    ...
    if (NODE_ID != 0 && rx_hdr.IDE == CAN_ID_STD && rx_hdr.StdId == (0x100 + NODE_ID))
    {
        ...
        // 5.3 step 2: the master's 0x101 target now drives the position loop.
        // Before, it was only latched into g_remote_pos while ControlTask kept
        // using the boot-default g_target_position, so CAN commands had no effect.
