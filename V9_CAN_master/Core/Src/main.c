...
int16_t g_target_speed = 0;        // target speed, set by the position loop
...
volatile float Kp = 2.0f;                   // speed-loop P gain
volatile float Ki = 0.1f;                   // speed-loop I gain
volatile float Kd = 0.0f;                   // speed-loop D gain
volatile uint8_t g_stall = 0;      // stall latch: set by DiagTask, cleared by new "pos" command

//=== position loop (outer) ===
volatile int16_t g_target_position = 200;    // position target, in encoder counts
volatile float Kp_pos = 0.2f;               // position-loop P gain

#define PID_PERIOD_MS  20
#define PWM_MAX        49
#define INTEGRAL_LIMIT 500
#define MAX_SPEED       80          // max speed limit (counts per 20ms)
#define DECEL_ZONE      150         // decel zone: slow the speed setpoint near target
#define DEAD_BAND       3           // dead band: ignore |pos error| < 3 counts
...
#define NODE_ID   0        // compile-time role: 0 = master, 1 = slave (same source, role at compile time)
#define NODE_COUNT 2       // protocol: 0x100+id master->node, 0x200+id node->master
...
volatile int16_t  g_slave_pos   = 0;  // master: latest slave feedback position (0x201)
volatile uint8_t  g_slave_stall = 0;  // master: slave's stall status bit (0x201 data[2])
volatile uint8_t  g_clr_stall_cnt = 0; // master: frames left sending flags bit1 (clear-stall)
/* USER CODE END PV */
...
void uart_print_enc(int16_t val)
{
    ...
    // same as uart_print_int, but append CRLF
    int start = 0, end = i - 1;
    while (start < end)
    {
        char tmp = buf[start];
...
// drive forward: PA7=1, PB1=0
HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);
HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_RESET);
__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, 0);

// zero encoder, start TIM2 encoder mode
__HAL_TIM_SET_COUNTER(&htim2, 0);
HAL_TIM_Encoder_Start(&htim2, TIM_CHANNEL_ALL);

g_last_enc = 0;
g_target_position = 200;           // initial position target
...
#if NODE_ID == 0
    // master: accept the first slave's feedback (0x201). mask=0 = exact match
    // on genuine STM32 (bit 1 = don't-care); on GD32-style clones the mask bit
    // meaning is inverted, so mask=0 degrades to accept-all and still passes
    // 0x201 - the same config is therefore safe on BOTH silicon families and
    // cannot false-negative a working board. (3-node: add a second filter bank
    // for 0x202.) This mirrors the slave's proven exact-match setup.
    can_f.FilterIdHigh = (uint16_t)(((0x200 + 1) << 5) & 0xFFFF);
    can_f.FilterIdLow  = 0;
...
void DisplayTask(void *argument)
{
    char cache[6][8] = {0};     // last rendered value strings (for change detection)
    char b[8];
    ...
    OLED_DrawString(1, 0,  "M-POS");
    OLED_DrawString(2, 0,  "M-TGT");
    OLED_DrawString(3, 0,  "M-SPD");
    OLED_DrawString(4, 0,  "M-PWM");
    OLED_DrawString(5, 0,  "S-POS");
    ...
    for (;;)
    {
        ...
        // --- 6 live lines: re-draw only when the rendered text changed ---
        ...
        // slave feedback: its encoder position latched from 0x201; if the
        // slave latched a stall, show it instead of a frozen number.
        if (g_slave_stall) { strcpy(b, "STALL "); }
        else               { fmt_signed(b, g_slave_pos, 6); }
        if (strcmp(b, cache[5]) != 0) { strcpy(cache[5], b); OLED_DrawString(5, 24, b); }
...
void CANTask(void *argument)
{
    ...
    for (;;)
    {
        ...
        if (NODE_ID == 0)
        {
            ...
            if (g_clr_stall_cnt)                 // bit1 = clear-stall: counts down
            {                                    // after a fresh "pos", so a stalled
                tx_data[2] |= 0x02;              // slave recovers without a power cycle
                g_clr_stall_cnt--;
            }
        }
        else
        ...
        // MASTER: periodic serial link-health line (free text; the upper
        // computer skips it, so the 5-value CSV protocol is unchanged).
        // rx growing => slave feedback is arriving; tx growing => our target
        // frame is being ACKed on the bus. This is the serial-visible half of
        // the link check for bare boards without an OLED.
        if (NODE_ID == 0)
        {
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcanp)
{
    ...
    if (NODE_ID == 0)
    {
        // Master: PC13 blinks per received feedback frame - the same visible
        // link indicator the slave has, for bare boards with no OLED/serial.
        // The exact-match 0x201 filter means every accepted frame IS feedback.
        // 0x201 payload: [0:1] slave encoder pos (int16 LE), [2] status bit0 = stall.
        g_slave_pos   = (int16_t)(rx_data[0] | (rx_data[1] << 8));
        g_slave_stall = rx_data[2] & 0x01;
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
    }
    else if (rx_hdr.IDE == CAN_ID_STD && rx_hdr.StdId == (0x100 + NODE_ID))
    {
